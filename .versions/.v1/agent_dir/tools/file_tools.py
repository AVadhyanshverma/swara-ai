...
WORKSPACE_BASE = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "agent_workspace"))

def get_session_dir():
    # Use a static session folder for the current run, based on date
    # Ideally, this would be passed dynamically by the chat engine
    session_id = time.strftime("%Y%m%d_%H%M%S")
    session_dir = os.path.join(WORKSPACE_BASE, f"session_{session_id}")
    if not os.path.exists(WORKSPACE_BASE):
        os.makedirs(WORKSPACE_BASE, exist_ok=True)
    # We will just use the WORKSPACE_BASE as the root, and tools can create session dirs inside it.
...
class CreateFileInput(BaseModel):
    filepath: str = Field(..., description="Relative path within the agent workspace.")

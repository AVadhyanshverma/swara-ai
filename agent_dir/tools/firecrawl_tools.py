...
def analyze_with_nim(text: str, prompt: str) -> str:
    ...
    try:
        ...
        resp = httpx.post("https://adhyanshverma-data-gen.hf.space/analyze", data={"text": combined}, timeout=120.0, headers={"Authorization": f"Bearer {HF_TOKEN}"})
...
def read_the_page(
    ...
    try:
        if extraction_prompt or extraction_schema:
            ...
            return json.dumps(response_model.model_dump(), indent=2, default=str)
        else:
            if not formats:
                formats = ["markdown"]
